...
var featureRegistry = []FeatureRecord{
	...
	},
	{
		...
		Status: StatusDisabledNeedBlocker,
		Reason: "TreeDB native conditional transactions currently return unsupported under the durable command-WAL profile",
		Evidence: []string{
	...
	},
	{
		...
		Status: StatusDisabledNeedBlocker,
		Reason: "Dgraph uses Badger entry UserMeta and ExpiresAt fields that do not have a TreeDB compatibility contract yet",
		Evidence: []string{
			"worker/treedb README compatibility inventory: Entry.UserMeta, Item.UserMeta, Entry.ExpiresAt",
		},
	},
	{
...
func (e *FeatureReadinessError) Error() string {
	...
	return fmt.Sprintf("%s: required feature set is not ready: %s", ErrUnsupportedFeature, strings.Join(parts, "; "))
}
